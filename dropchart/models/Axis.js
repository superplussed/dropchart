define('Axis', ['utils'], function(utils) {
  ...
  Axis.prototype.addXAxisLine = function() {
    var yVal = this.canvas.formatY(this.yScale(0));
    this.rootGroup.append('svg:line')
      .attr('class', 'x-line')
  ...
  Axis.prototype.assignRangesFromDimensions = function() {
    ...
    that.xScale = d3.scale.linear().range([0, that.canvas.width]);
  ...
  Axis.prototype.addYAxis = function() {
    ...
    this.root.append('g')
        .attr('class', 'y axis')
        .attr('transform', 'translate(' + canvas.translateX(this.options.yAxisPosition) + ',' + canvas.translateY(this.options.marginTop) + ')')
        .call(this.yAxis);
  };
  ...
  Axis.prototype.addXAxisLabels = function() {
    ...
    this.xAxisLabel = this.bars.append('svg:rect')
      ...
      .attr('width', canvas.formatX(this.barWidth))
      .attr('height', canvas.formatY(this.options.xAxisLabelHeight))
      ...
      .attr('x', function(d, i) { return canvas.formatX(that.xScale(i)); })
      .attr('y', canvas.formatY(this.options.xAxisPosition));
    ...
    this.xAxisLabelText = this.bars.append('svg:text')
      ...
      .attr('x', function(d, i) {
        return canvas.formatX(that.xScale(i) + (that.barWidth / 2));
      })
  ...
  Axis.prototype.calculateBarSize = function() {
    var that = this;
    that.barWidth = utils.roundNumber(((this.canvas.width) / that.data.length) * that.options.barWidthModifier, 2);
  };
